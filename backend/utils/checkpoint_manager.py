...
class CheckpointManager:
    ...
    def __init__(self):
        self._checkpoint = {
            ...
            "in_progress": set(),
            "file_positions": {},
            "statistics": {"total_processed": 0, "total_failed": 0, "retry_count": 0},
        }
        ...
        async with self._lock:
            try:
                if status == FileStatus.IN_PROGRESS:
                    ...
                    self._checkpoint["in_progress"].discard(file_path)
                    failed_info = self._checkpoint["failed_files"].get(
                        file_path,
                        FailedFileInfo(
                            retry_count=0, last_error="", last_position=Position(0, 0)
                        ),
                    )
                    ...
                    self._checkpoint["failed_files"][file_path] = FailedFileInfo(
                        ...
                        last_position=position
                        if position
                        else failed_info.last_position,
                    )
    ...
    async def get_last_position(self, file_path: str) -> Position:
        ...
        async with self._lock:
            return self._checkpoint["file_positions"].get(
                file_path, Position(line=0, character=0)
            )
    ...
    async def load_state(self, checkpoint: Dict) -> None:
        ...
        async with self._lock:
            try:
                self._checkpoint["processed_files"] = set(
                ...
                self._checkpoint["file_positions"] = checkpoint.get(
                    "file_positions", {}
                )
                self._checkpoint["statistics"] = checkpoint.get(
                    "statistics",
                    {"total_processed": 0, "total_failed": 0, "retry_count": 0},
                )
                logger.info(
                    f"Loaded checkpoint with {len(self._checkpoint['processed_files'])} processed files"
                )
            except Exception as e:
