...
async def test_analyze_empty_directory(package_analyzer, tmp_path):
    with patch(
        "algorithms.package_analyzer.LanguageDetector.detect",
        return_value=Mock(value="python"),
    ):
        updates = []
...
async def test_analyze_with_python_file(package_analyzer, tmp_path, mock_lsp_client):
    ...
    patches = [
        patch(
            "algorithms.package_analyzer.LanguageServerPoolManager.get_client",
...
async def test_process_references_file(package_analyzer, tmp_path, mock_lsp_client):
    ...
    package_analyzer.symbol_registry["test_id"] = {
        ...
        "file": str(test_file),
        "node": {"id": "test_id", "name": "TestClass", "kind": "class"},
    }
...
async def test_failed_file_processing(package_analyzer, tmp_path):
    ...
    with patch(
        ...
        update = await package_analyzer.process_structure_file(
            test_file, package_analyzer._create_project_node(tmp_path)
        )
