...
import asyncio
from utils.checkpoint_manager import CheckpointManager, FileStatus, Position
...
async def test_update_file_status(checkpoint_manager: CheckpointManager):
    ...
    await checkpoint_manager.update_file_status(
        file_path, FileStatus.IN_PROGRESS, position=position
    )
    in_progress = await checkpoint_manager.get_in_progress_files()
    assert file_path in in_progress

    # Test COMPLETED status
    await checkpoint_manager.update_file_status(file_path, FileStatus.COMPLETED)
...
async def test_failed_file_handling(checkpoint_manager):
    ...
    # Test FAILED status
    await checkpoint_manager.update_file_status(
        file_path, FileStatus.FAILED, error=error_msg, position=position
    )
...
async def test_statistics(checkpoint_manager):
    ...
    await checkpoint_manager.update_file_status(
        "failed.py", FileStatus.FAILED, error="Error 1"
    )
    await checkpoint_manager.update_file_status(
        "failed.py", FileStatus.FAILED, error="Error 2"
    )
...
async def test_concurrent_access():
    ...
    async def update_file(file_path):
        await manager.update_file_status(file_path, FileStatus.IN_PROGRESS)
        await asyncio.sleep(0.1)
        await manager.update_file_status(file_path, FileStatus.COMPLETED)
...
async def test_error_recovery(checkpoint_manager):
    ...
    await checkpoint_manager.update_file_status(file_path, FileStatus.IN_PROGRESS)
