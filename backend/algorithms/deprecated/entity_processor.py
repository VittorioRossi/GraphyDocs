...
class EntityProcessor:
    ...
    async def _process_config_file(
        ...
        return {
            ...
            "type": "config",
            "name": file_path.name,
            "data": {"type": config_type, "path": str(file_path)},
        }

    async def _process_source_file(
        self, file_path: Path, project_name: str, session: AnalysisSession
    ) -> List[Dict]:
        nodes = []
                    ...
                    nodes.append(
                        {
                            ...
                            "data": {"type": entity_data.kind, "path": str(file_path)},
                        }
                    )
            ...
            try:
                ...
                for ref in refs:
                    ref_symbol = next(
                        (
                            s
                            for s in session.symbol_cache.values()
