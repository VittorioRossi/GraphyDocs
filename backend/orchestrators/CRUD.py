    ...
    try:
        ...
        async with neo4j.session() as session:
            ...
            neo4j_stats = (
                ...
                if record
                else {"total_nodes": 0, "total_relationships": 0}
            )

        return {
            "postgresql": dict(stats)
            if stats
            else {
                "total_projects": 0,
                "total_jobs": 0,
...
async def delete_project(
    ...
    try:
        ...
        project_check = await db.execute(
            text("SELECT name FROM projects WHERE id = :id"), {"id": str(project_id)}
        )
        ...
        await db.execute(
            text("DELETE FROM jobs WHERE project_id = :id"), {"id": str(project_id)}
        )
        await db.execute(
