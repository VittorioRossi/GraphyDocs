...
class ProjectLoadingOrchestrator:
    ...
    async def clone_github_repository(
        self, request: GitProjectRequest, db: AsyncSession
    ) -> Dict[str, str]:
        correlation_id = str(uuid.uuid4())
        self.logger.debug(
            f"[{correlation_id}] Starting repository clone for URL: {request.url}"
        )
        ...
        try:
            ...
            config = GitConfig(access_token=request.token, branch="main")
            ...
            try:
                ...
                clone_path = git_ops.clone_repository(
                    request.url, local_path=project_path, return_files=False
                )
            ...
            self.logger.error(
                f"[{correlation_id}] Database error: {str(e)}\n{traceback.format_exc()}"
            )
            await db.rollback()
            raise HTTPException(status_code=500, detail="Database error occurred")
        except Exception as e:
            self.logger.error(
                f"[{correlation_id}] Error during clone: {str(e)}\n{traceback.format_exc()}"
            )
            await db.rollback()
            raise HTTPException(
    ...
    async def unzip_archive(
        ...
        try:
            with tempfile.NamedTemporaryFile(delete=False) as temp_file:
                ...
                extract_path = self.fs_ops.unzip_folder(temp_file.name, project_path)
    ...
    async def list_file_system_elements(self, db: AsyncSession) -> List[Dict[str, str]]:
        ...
        try:
            ...
            self.logger.debug(
                f"[{correlation_id}] Found {len(list(projects))} projects"
            )
            ...
            for project in projects:
                if not await self.validate_project_id(project.id, db):
                    self.logger.error(
                        f"[{correlation_id}] Invalid project ID: {project.id}"
                    )
                    continue

                if os.path.exists(project.path):
                    self.logger.debug(
                        f"[{correlation_id}] Processing project: {project.name}"
                    )
                    ...
                    self.logger.warning(
                        f"[{correlation_id}] Project path does not exist: {project.path}"
                    )

            self.logger.info(
                f"[{correlation_id}] Successfully listed {len(elements)} elements"
            )
            return elements
            ...
            self.logger.error(
                f"[{correlation_id}] Error listing elements: {str(e)}\n{traceback.format_exc()}"
            )
    ...
    async def validate_project_id(self, project_id: str, db: AsyncSession) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Database error validating project {project_id}: {str(e)}"
            )
            return False
        except Exception as e:
...
@router.post("/git")
async def clone_github_repository(
    request: GitProjectRequest, db: AsyncSession = Depends(get_db)
):
    logger.info(f"Received request to clone repository: {request.token}")
    return await orchestrator.clone_github_repository(request, db)


@router.post("/zip")
async def unzip_archive(
    zip_file: UploadFile = File(...), db: AsyncSession = Depends(get_db)
):
    try:
...
async def list_file_system_elements(db: AsyncSession = Depends(get_db)):
    try:
        ...
        logger.error(
            f"Unexpected error in list endpoint: {str(e)}\n{traceback.format_exc()}"
        )
...
async def validate_project(project_id: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if not is_valid:
            raise HTTPException(
                status_code=404, detail="Project not found or inaccessible"
            )
